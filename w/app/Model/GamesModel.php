...
class GamesModel extends \W\Model\Model
{
	...
	*/ 
	...
	public function jointureCourtsGames($date) {
		...
		$sql = 'SELECT DISTINCT court_id, description, name, parking, picture, address, postal_code, city, net FROM '.$this->table.' AS g INNER JOIN courts AS c ON g.court_id = c.id WHERE city LIKE :recherchecity OR postal_code LIKE :recherchepostalcode AND date LIKE :date ORDER BY g.date ASC';
