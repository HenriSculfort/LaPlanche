...
class CourtsModel extends \W\Model\Model
{
	...
	*/ 
	...
	public function leftJoinCourtsGames ($date) {
		...
		$sql = ('SELECT * FROM ' .$this->table. ' WHERE id NOT IN ( SELECT CONCAT(court_location) AS id_terrains FROM games WHERE games.date = :date) AND city LIKE :recherchecity OR postal_code LIKE :recherchepostalcode'); 
